...
enum json_code_t json_string_parse(json_string_t *jstr, cnst_seg_t *next_chunk)
{
...
parse:
    /* this section handles the unescaped input */
    {
        const char *cur, *end;
        size_t len;
        cnst_seg_get_start_and_end(next_chunk, &cur, &end);
        assert(cur < end);
        ...
        do {
            if (*cur == '"')
                ...
                if (cur > next_chunk->start) {
                    ...
                    cnst_seg_trim_front(next_chunk, len);
                }
                goto escape_seq;
            }
        } while (++cur < end);
...
escape_seq:
    ...
    if (jstr->escape_seq_len == 0) {
        ...
        cnst_seg_trim_front(next_chunk, 1);
    }
...
enum json_code_t json_number_parse(json_number_t *jnum, cnst_seg_t *next_chunk)
{
...
enum json_code_t json_value_parse(json_value_t *jval, cnst_seg_t *next_chunk)
{
    enum json_code_t ret;
    assert(next_chunk->size > 0);  /* always expect some input */
